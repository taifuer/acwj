...
数组类似于指针，因为我们可以使用“[]”语法对指针和数组解引用，以获得对特定元素的访问。我们可以使用数组的名称作为“指针”，并将数组的基保存为指针。我们可以得到数组中元素的地址。但我们不能做的一件事是用指针“覆盖”数组的基地址：数组的元素是可变的，但数组的基地址是不可变的。
...
现在我们知道了数组的大小，我们可以修改`cgglobsym()`在汇编器中分配这个空间：
