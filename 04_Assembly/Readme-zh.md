...
`interpretAST()` 函数以深度优先的方式遍历给定的 AST 树。它先计算左子树，然后是右子树。最后，它使用当前树节点的 `op` 值对这些子节点进行操作。
...
`genAST()` 只计算给它的表达式的值。我们需要把最后的计算结果打印出来。我们还需要用一些前导代码（preamble，前导码）和一些尾随代码（postamble，后同步码）来包装生成的汇编代码。这可以用 `gen.c` 中的另一个函数来完成：
...
我不会一一介绍代码，因为它很简单，但会进行一些错误检查。现在，如果寄存器用完，程序就会崩溃。稍后，我将处理空闲寄存器用完的情况。
...
Linux x86-64 希望函数的第一个参数在 `%rdi` 寄存器中，因此我们在 `call printint` 之前将寄存器移到 `%rdi` 中。
...
太棒了。我们现在有了一个合法的编译器：一个程序接受一种语言的输入，然后生成另一种语言的翻译。

我们仍然需要将输出组装成机器代码，并将其与支持库链接起来，但这是我们目前可以手动执行的工作。稍后，我们将编写一些代码来自动完成这项工作。

## 总结与展望

从解释器更改为通用代码生成器是很简单的，但是我们必须写一些代码来生成真正的汇编输出。为此，我们必须考虑如何分配寄存器：目前，我们有一个简单的解决方案。我们还必须处理一些 x86-64 的奇怪问题，比如 `idivq` 指令。

我还没有提到的是：为什么要为一个表达式生成 AST 呢？当然，当我们在 Pratt 解析器中遇到一个 `+` 标记时，我们可以调用 `cgadd()`，其他运算符也是如此。我要让你去思考这个问题，但我会在一两步后回来。

在编译器编写之旅的下一部分，我们将在语言中添加一些语句，以便它开始类似于一种合适的计算机语言。
