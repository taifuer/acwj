...
现在我们有了两种表达式：加法表达式和乘法表达式。注意，语法现在强制数字只能作为乘法表达式的一部分。这就迫使 `*` 和 `/` 操作符与两边的数字绑定得更紧密，因此具有更高的优先级。
...
答案是使用 `multiplicative_expr()` 函数处理 `*` 和 `/` 操作符，使用 `additive_expr()` 函数处理优先级较低的 `+` 和 `-` 操作符。

这两个函数都要读入一些东西和一个操作符。然后，当后面的操作符具有相同的优先级时，每个函数将解析更多的输入，并用第一个操作符组合左右两半。
...
在一开始，我们立即调用 `multiplicative_expr()`，以防第一个操作符是高优先级的 `*` 或 `/`。该函数只会在遇到低优先级的 `+` 或 `-` 操作符时返回。

因此，当我们执行  `while`  循环时，我们知道我们有一个 `+` 或 `-` 操作符。循环直到输入中没有 token，即当我们碰到 `T_EOF` 标记时。

在循环内部，我们再次调用 `multiplicative_expr()`，以防后面操作符的优先级高于我们。同样，当不存在高优先级的操作符时，它将返回。

一旦我们有了左、右子树，我们就可以把它们和上次循环中得到的操作符结合起来。如此重复，所以如果我们有表达式 `2 + 4 + 6`，我们将得到 AST：
...
但是如果 `multiplicative_expr()` 有自己的更高优先级的操作符，我们将组合子树和其中的多个节点。
...
我们也只在操作符位于高优先级时进行循环。比如 `*` 和 `/` 操作符。一旦我们遇到一个低优先级操作符，我们就简单地返回到目前为止我们已经建立的子树。这又回到 `additive_expr()` 来处理低优先级操作符。
...
假设输入是 `234 101 + 12`。我们可以扫描前两个 token。但是，如果我们只是使用 `OpPrec[]` 来获取第二个 token `101` 的优先级，那么就不会注意到它不是一个操作符。因此，`op_precedence()` 函数强制使用正确的语法。
...
在 Pratt 解析器中，当下一个操作符的优先级高于当前 token 时，我们不只是使用 `primary()` 获取下一个整型字面量，而是使用 `binexpr(OpPrec[tokentype])` 调用自己来提高操作符的优先级。
...
这将是一个具有许多节点和运算符的子树，其优先级高于调用我们的运算符，也可能是一个与我们优先级相同的操作符的单个整型字面量。
