...
在编译器编写旅程的这一部分，我将介绍解析器的基础知识。正如我在第一部分中提到的，解析器的工作是识别输入的语法和结构元素，并确保它们符合语言的语法。
...
让我们在输入 `2 + 3 - 5 T_EOF ` 上运行这个函数，其中 `T_EOF` 是一个反映输入结束的 token。我会为每次对 expression() 的调用编号。
...
为了进行语义分析，我们要么解释已识别输入的代码，要么将其转换为另一种格式，例如汇编代码。在本部分中，我们将为输入构建一个解释器。但要实现这一点，我们首先要将输入转换为[抽象语法树](https://en.wikipedia.org/wiki/Abstract_syntax_tree)，也称为 AST (Abstract Syntax Trees)。
...
现在，我们可以重用来自扫描器的 token 值作为 AST 节点的操作值，但我喜欢将 token 和 AST 节点的概念分离开来。因此，首先，我会有一个函数将 token 值映射为 AST 节点的操作值。这和解析器的其余部分在 `expr.c` 中:
...
注意，在这个简单的解析器代码中，没有任何地方处理不同的操作符优先级。就目前的情况而言，代码将所有运算符视为具有同等的优先级。如果你按照代码解析表达式  `2 * 3 + 4 * 5`，你会看到它构建了这个 AST:
