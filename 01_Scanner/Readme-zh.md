...
其中，`token` （标记）字段可以是以下值之一（来自 `defs.h`）：
...
当标记是 `T_INTLIT`（即整数）时，`intvalue` 字段将保存我们扫描的整数的值。

## `scan.c` 中的函数

`scan.c`  文件保存了词法扫描器的函数。我们将从输入文件中一次读入一个字符。然而，如果在输入流中读得太远（多读了字符），有时会需要“放回”一个字符。我们还希望跟踪当前所在的行，以便在调试信息中打印行号。所有这些都是由 `next()` 函数完成的：
...
所有 C 文件都将包含此内容，其中 `extern_` 被替换为 `extern`。但是 `main.c` 会去掉 `extern_`；因此，这些变量将“属于” `main.c`。
...
简单的单字符标记就是这样：对于每个识别的字符，将其转换为一个标记。你可能会问：为什么不将识别的字符放入 `struct token` 中？答案是，稍后我们将需要识别多字符标记，如 `== ` 和关键字，如 `if` 和 `while`。因此，使用标记值的枚举列表会更容易。
...
就在最后，你是否注意到了 `putback(c)` 的调用？此时我们发现了一个不是十进制数字的字符。我们不能简单地丢弃它，相反，我们应该将它放回输入流中，以便稍后使用。
...
`main.c` 中的代码让上述扫描器工作。`main()` 函数打开一个文件，然后扫描其中的标记：
...
单字符标记很容易扫描，但多字符标记就有点难了。但在最后，`scan()` 函数会将输入文件中的下一个标记返回到 `struct token` 变量中：
...
在编译器编写过程的下一部分中，我们将构建一个递归下降解析器来解释输入文件的语法，计算并且打印出每个文件的最终值。
