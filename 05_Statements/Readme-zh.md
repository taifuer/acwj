...
我们还需要一个函数来识别语言中的关键字。一种方法是使用一个关键字列表，遍历列表并使用 `strcmp()` 将每个关键字都与  `scanident()` 的缓冲区 `buf` 进行比较 。来自 `SubC` 的代码有一个优化：在执行 `strcmp()` 之前匹配第一个字母。这加快了对数十个关键字的比较。现在我们不需要这种优化，但我已经把它放进去了：
